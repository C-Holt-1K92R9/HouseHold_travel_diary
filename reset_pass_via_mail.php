...
$check=$conn->query("SELECT * from user where verification_token = '$token' ");
if ($check->num_rows >0){
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST['password'])){
            ...
            $check=$conn->query("SELECT * from user where verification_token = '$token' ");
                ...
                if ($check->num_rows >0){
                    $check->close();
                    $conn->query("UPDATE user SET password= '$new_pass', verification_token = NULL where verification_token = '$token'");
                    header("Location: success.php?head=Password Reset Successful! & msg=Your password has been successfully reset! You can now log into your account. &btn=Back to Login");
                    exit();
                }
...
<body>
  <div class="login-container">
    ...
    <form id="loginForm" class="login-form" action="reset_pass_via_mail.php?token=<?=$token?>" method="POST">
